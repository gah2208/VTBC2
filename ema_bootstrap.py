...
def initialize_ema_engine(client, expiry, symbol="SPX", rebuild_depth=None):  #2
    """
    ...
    3) Otherwise, poll broker for 1-minute SPX data (>= EMA_REBUILD_DEPTH minutes)
    ...
    # NEW: configurable rebuild depth (1-minute bars)
    cfg_rebuild_depth = config.get("EMA_REBUILD_DEPTH", 120)
    rebuild_depth = cfg_rebuild_depth if rebuild_depth is None else rebuild_depth
    ...
    # ------------------------------------------------------------
    # 2) Fallback: broker 1-minute data (need >= EMA_REBUILD_DEPTH)
    # ------------------------------------------------------------
    # OLD (COMMENTED OUT): wrapper defaulted internally to 60
    # prices = get_minute_prices_for_rebuild(client, expiry, symbol=symbol)  #2

    # UPDATED: request depth explicitly so fetch matches rebuild requirement
    # UPDATED: disable quote-poll fallback for bootstrap to avoid startup hangs
    prices = get_minute_prices_for_rebuild(
        client,
        expiry,
        symbol=symbol,
        barsback=rebuild_depth,
        allow_fallback_polling=False
    )  #2
    ...
    # UPDATED: configurable EMA_REBUILD_DEPTH (1-minute bars)
    if not prices or len(prices) < rebuild_depth:
        raise Exception(f"Not enough historical data to initialize EMAs (need ≥{rebuild_depth} 1-minute bars)")

    prices = prices[-rebuild_depth:]
...
def reseed_after_gap(client, expiry, ema_engine, symbol="SPX"):  #2
    """
    Rebuild EMAs after a trading halt or multi-day outage.

    Design:
    - Poll broker for last EMA_REBUILD_DEPTH minutes of 1-minute SPX data.
    - Rebuild EMAs from that window.
    """

    config = load_merged_config()
    rebuild_depth = config.get("EMA_REBUILD_DEPTH", 120)

    # OLD (COMMENTED OUT): wrapper defaulted internally to 60
    # prices = get_minute_prices_for_rebuild(client, expiry, symbol=symbol)  #2

    # UPDATED: request depth explicitly so fetch matches rebuild requirement
    # UPDATED: disable quote-poll fallback during reseed to avoid long blocking
    prices = get_minute_prices_for_rebuild(
        client,
        expiry,
        symbol=symbol,
    ...
    # UPDATED: configurable EMA_REBUILD_DEPTH (1-minute bars)
    if not prices or len(prices) < rebuild_depth:
        raise Exception(f"Not enough historical data to reseed EMAs (need ≥{rebuild_depth} 1-minute bars)")
